...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "Muskan AI + Clean Media Downloader",
    longDescription: "AI chat with Muskan and automatic music/video downloader",
    category: "ai",
    guide: {
      en: "{p}muskan <baat karein ya gaana/video maangein>"
    }
  },
  ...
  onStart: async function ({ api, event, args, message, usersData }) {
    ...
    if (!cleanedMsg) return message.reply("Bolo na Jaan, kya baat karni hai? 😘");
    ...
    if (isVideoReq || isAudioReq || isUrl) {
      ...
      try {
        ...
        processingMsg = await message.reply("✅ Apki Request Jari Hai Please Wait...");
        ...
        if (!query) {
          ...
          api.setMessageReaction("❌", messageID, () => {}, true);
          return message.reply("Naam to batao kya download karun? 🥺");
        }
        ...
        if (!video) {
          if (processingMsg) api.unsendMessage(processingMsg.messageID).catch(() => {});
          api.setMessageReaction("❌", messageID, () => {}, true);
          return message.reply("Maafi, ye video ya song nahi mila 🥺💔");
        }
        ...
        if (isVideo) {
          ...
          await message.send({ attachment: fs.createReadStream(cachePath) });
        }
        return;
        ...
        api.setMessageReaction("❌", messageID, () => {}, true);
        return message.reply("Server thoda thak gaya hai ya error agaya hai 🥺💔");
      } finally {
        if (processingMsg) api.unsendMessage(processingMsg.messageID).catch(() => {});
...
Context:\n${chatMemory.history[threadID].join("\n")}\nMuskan:`;
    ...
    try {
      ...
      return message.reply("Mera net thoda slow chal raha hai, baad mein baat karte hain 🥺");
    }
  },
  ...
  onChat: async function ({ api, event, message }) {
    ...
    if ((messageReply && messageReply.senderID == api.getCurrentUserID()) || body.toLowerCase().startsWith("muskan")) {
      ...
      return this.onStart({ api, event, args, message, usersData: global.GoatBot.usersData });
    }
  }
};
